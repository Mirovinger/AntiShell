...
if (!defined('AS_CHARSET')) {
	define('AS_CHARSET', 'utf-8');
}
?>
	<!DOCTYPE html>
	<html>
	<head>
		...
		<style>
			...
			li li, li h3 { color: #34495e; }
			textarea { width: 800px; margin-bottom: 10px; vertical-align: top; -webkit-transition: height 0.2s; -moz-transition: height 0.2s; transition: height 0.2s; outline: none; display: block; color: #f39c12; padding: 5px 10px; font: normal 14px/20px Consolas, 'Courier New', monospace; background-color: #2c3e50; white-space: pre-wrap; word-break: break-all; word-wrap: break-word; text-shadow: none; border: none; border-left: solid 3px #f39c12; }
			...
			.label { float: left; width: 300px; padding-right: 10px; line-height: 32px; text-align: right; }
			.control { width: 322px; float: left; }
			.control input[type="text"] { width: 300px; margin-bottom: 2px; }
			.form-field-large .label { width: 100px; }
	...
	<body>
	<header>
		<h1 class="ta-center"><span class="red big">AntiShell</span> <br><span class="blue">Мастер установки скрипта для предупреждения взлома Вашего сайта</span>
		</h1>
		<hr>
	</header>
	<section>
 ...
 * @return string
 */
function curlGet($url, $curlopt = array()) {
...
function antiShellInstaller() {
	...
	/** @var int $version */
	$version = ($ver) ? json_decode($ver, true) : 0;
	...
	if (!empty($_POST['install'])) {
		...
		$as = curlGet($github . 'src/source.php');
		...
		$as_config_values = array(
			...
			($_POST['send_domain']) ? true : 0,
			($_POST['statistics_login']) ? $_POST['statistics_login'] : "",
		);
		...
		if (!file_exists($docroot . $script_filename)) {
			...
			$output .= '<p>Мастер установки не смог получить доступ к файлу скрипта. Это значит что либо нет прав на запись в указанную папку, либо не верно указан путь к корню сайта.</p>';
...
HTML;
	...
	elseif (!empty($_POST['sendmail'])) {
		 ...
		 * @return bool
		 */
		function mailfromsite(
			$buffer,
			$subject,
			$email,
			$text = "Установлен AntiShell скрипт",
			$from_email = 'noreply@antishell.ru'
		) {
...
<body style="background-color:#ecf0f1; margin: 0; padding:0;">
	<div style="max-width: 800px; margin: 0 auto;">
		...
		<div style="background-color:#ecf0f1; font:normal 16px 'Trebuchet MS', Arial, sans-serif; color:#7f8c8d; margin:0; padding: 0 20px 20px 20px;">
			...
			<div class="descr">
				<form method="POST">
					...
					<div class="form-field clearfix">
						<div class="label">Путь к корню сайта</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Путь к файлу скрипта от корня сайта</div>
						<div class="control">
							<input type="text" name="script_filename" value="/antishell.php">
							<small class="red">Обязательно изменить имя и путь файла!</small>
							<small class="gray"><br>По указанному адресу будет создан файл с кодом скрипта. Папка в которой будет создаваться скрипт. Рекоендуется прятать скрипт поглубже, а ссылку не афишировать. <br>К примеру так: <code>/engine/classes/min/lib/old.php</code></small>
						</div>
					</div>

					<div class="form-field clearfix">
						<div class="label">Имя сайта</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Начальный путь проверки</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Название файла со снимком системы</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Автоматическое создание снимка</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Расширения файлов, которые необходимо проверять</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Расширения файлов, которые проверяться не будут</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Список папок, которые не надо проверять</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Путь к иконкам-индикаторам
					...
					<div class="form-field clearfix">
						<div class="label">Отправлять статистку</div>
						<div class="control">
					...
					<div class="form-field clearfix">
						<div class="label">Добавлять в статистку имя домена</div>
						<div class="control">
							<input type="checkbox" value="1" name="send_domain" id="send_domain" checked class="checkbox"><label for="send_domain"><span></span> Да</label> <br>
							<small class="gray">Разрешить отправку в статистику имя проверяемого домена.
							<br>В будущем можно бдет получить статистику по вашему домену.
							<br>Домен не публикуется, а статистику по домену может получить только владелец домена или пользователь, назвавший логин из поля ниже!
							</small>
						</div>
					</div>

					<div class="form-field clearfix">
						<div class="label">Логин для сбора статистики</div>
						<div class="control">
							<input type="text" name="statistics_login" value="">
							<small class="gray">Логин, для которого будет собираться статистика сканирования.
							<br>Рекомендуется указывать, если сайтов несколько, тогда можно будет получить статистику сразу по всем своим сайтам.
							<br>Допускается использование email-адреса
							</small>
						</div>
					</div>
